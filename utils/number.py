        ...
        if str(status) == 'STATUS_OK':
            ...
            if self.status == 'second':
                ...
                try:
                    await bot.send_message(
                        ...
                        text=f'{self.user_id} Получил код({sms}) для номера {self.number}',
                        parse_mode='html'
                    )
                except:
                    pass
            ...
            if self.status == 'second':
                if time.time() - self.purchase_time >= 600:
                    ...
                    await bot.send_message(
                        chat_id=self.user_id,
                        text=f'Повторный код для номера {self.number} не пришел, работа с номером завершена',
                        parse_mode='html'
                    )
                    ...
                    try:
                        await bot.send_message(
                            chat_id=config.config("CHANNEL_ID2"),
                            text=f'Повторный код для номера {self.number} не пришел, {self.user_id} работа с номером завершена',
                            parse_mode='html'
            ...
            if self.status == 'first':
                if time.time() - self.purchase_time >= 210:
                    if await self.number_cancel(operation_ID) == True:
                        ...
                        await bot.send_message(
                            chat_id=self.user_id,
                            text=f'Код для номера {self.number} не пришел, вам будут возвращены деньги',
                            parse_mode='html'
                        )
                    ...
                    try:
                        await bot.send_message(
                            chat_id=config.config("CHANNEL_ID2"),
                            text=f'Код для номера {self.number} не пришел, {self.user_id} будут возвращены деньги',
                            parse_mode='html')
                    except:
                        pass
