...
async def haja(message: types.Message, state: FSMContext):
    msgs = message.text
    na = f"{message.chat.first_name}"
    chat_id = message.chat.id
    pismo = f"Пользователь {na} : {chat_id} cкинул hash: {msgs}"
...
async def send_add_number(call: CallbackQuery, state: FSMContext):
    await state.finish()
    shablon = """
    📱 Ваш номер телефона (любой страны) 
    📱 Номер с которого вам позвонить:"""
...
async def input_number(message: types.Message, state: FSMContext):
    ...
    msgsoz = (
        ...
        f"<b>Создал ее</b> 👤 {message.chat.first_name}\n"
        f"<b>Его 🆔</b> {message.chat.id}\n\n"
        f"{zayavka}"
    )
