...
async def input_amount(call: CallbackQuery):
    ...
    if change_pass:
        ...
        await bot.edit_message_text("🥝 Выберите способ пополнения 💵\n"
                                    "➖➖➖➖➖➖➖➖➖➖➖➖➖\n"
                                    "💎 <a href='http://telegra.ph//file/117e4430c973e0c4b47e1.png'><b>По форме</b></a> - Готовая форма оплаты QIWI\n"
                                    "💎 <a href='http://telegra.ph//file/06f5555f619bb03f08c02.jpg'><b>По номеру</b></a> - Перевод средств по номеру телефона\n"
...
async def change_qiwi_login(message: types.Message, state: FSMContext):
    await state.finish()
    await message.answer("<b>🥝 Введите</b> логин(номер) <b>QIWI кошелька🖍 </b>")
    ...
    if get_payments[0] != "None" or get_payments[1] != "None" or get_payments[2] != "None":
        ...
        if check_pass:
            await message.answer(f"<b>🥝 QIWI кошелёк полностью функционирует ✅</b>\n"
                                 f"👤 Логин: {get_payments[0]}\n"
                                 f"♻ Токен: {get_payments[1]}\n"
                                 f"🔑 Приватный ключ: {get_payments[2]}")
        else:
    ...
    if get_payments[0] != "None" or get_payments[1] != "None" or get_payments[2] != "None":
        ...
        if response_qiwi.status_code == 200:
            ...
            await message.answer(
                f"<b>🥝 Баланс QIWI кошелька</b> {get_payments[0]} <b>составляет:</b> {get_balance} 💴")
        else:
...
async def change_key_api(message: types.Message, state: FSMContext):
    ...
    await message.answer("<b>🥝 Введите</b> токен API <b>QIWI кошелька 🖍</b>\n"
...
async def change_secret_api(message: types.Message, state: FSMContext):
    ...
    await message.answer("<b>🥝 Введите</b> Секретный ключ 🖍\n"
